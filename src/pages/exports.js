...
export const exportarExcel = (leituras, chuvas) => {
  ...
  const resumo = dias.map(d => ({
    ...
    "Manha Temp (C)":   d.manha_temp ?? "",
    "Manha Umidade (%)":d.manha_umi  ?? "",
    "Tarde Temp (C)":   d.tarde_temp ?? "",
    "Tarde Umidade (%)":d.tarde_umi  ?? "",
    "Noite Temp (C)":   d.noite_temp ?? "",
    "Noite Umidade (%)":d.noite_umi  ?? "",
